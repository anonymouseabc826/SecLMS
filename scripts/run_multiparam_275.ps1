# scripts/run_multiparam_275.ps1 - FW 0.1.275 board 9-param sweep (no W8; P2+P3 hw values).
# Per combo: reflash board (reset hits) -> full KAT+LM-OTS+LMS suite -> archive log.
# Usage:
#   powershell -File scripts/run_multiparam_275.ps1
#   powershell -File scripts/run_multiparam_275.ps1 -Combos W4_H5,W1_H10
param(
...
$bit = "build/bitstreams/shake_fw0275b_fifofix_20260817.bit"
$platform = "shake256"
$outdir = "build/multiparam_275"
...
foreach ($combo in $Combos.Split(',')) {
    ...
    Get-Process | Where-Object { $_.ProcessName -match 'hw_server|cs_server' } | Stop-Process -Force -ErrorAction SilentlyContinue
