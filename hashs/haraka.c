...
/* REVIEW B04-R6: LMS only uses haraka_S(); the incremental interfaces and haraka512/haraka256(_sk) etc. are
 * upstream dead code kept intact, not trimmed (PC-only; firmware no longer links Haraka). */
                            ...
                            unsigned char p)
{
    ...
    unsigned char t[HARAKAS_RATE]; /* REVIEW B04-R7: the original VLA t[r] (r is always 32) changed to a fixed size */
...
/* ---- The following interfaces are kept fully intact upstream but unused by LMS (REVIEW B04-R6):
 * haraka_S_inc_* (incremental sponge) and haraka512/haraka256/haraka256_sk (PRF).
 * The LMS path only uses haraka_S (sponge, rate=32, domain 0x1F). ---- */
...
/* haraka512/haraka256/haraka256_sk are upstream-kept interfaces, unused by LMS (REVIEW B04-R6). */
