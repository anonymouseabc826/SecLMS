...
void KeccakP1600_ExtractAndAddLanes(const KeccakP1600_plain32_state *state, const unsigned char *input, unsigned char *output, unsigned int laneCount)
{
    ...
    for(lanePosition=0; lanePosition<laneCount; lanePosition++) {
        ...
        laneAsBytes[6] = (high >> 16) & 0xFF;
        laneAsBytes[7] = (high >> 24) & 0xFF;
        /* REVIEW B04-R1: the big-endian branch is unreachable in this project (PC/RV32 are both little-endian, and ExtractAndAdd*
         * is not called by xkcp_shake.c); the upstream structure is kept but a latent index error is fixed -- the 2nd half-lane
         * must XOR input[1] (originally wrote input[0], inconsistent with the LE branch's "whole 8-byte lane XOR" semantics).
         * Note that the uint8_t*->uint32_t* cast is the upstream original style (alignment/aliasing UB); not refactored until enabled. */
        ((uint32_t*)(output+lanePosition*8))[0] = ((uint32_t*)(input+lanePosition*8))[0] ^ (*(const uint32_t*)(laneAsBytes+0));
        ((uint32_t*)(output+lanePosition*8))[1] = ((uint32_t*)(input+lanePosition*8))[1] ^ (*(const uint32_t*)(laneAsBytes+4));
    }
#endif
}
/* ---------------------------------------------------------------- */
